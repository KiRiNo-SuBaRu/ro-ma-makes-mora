...
exports.decideEffectiveRoleFromScopeFlag = decideEffectiveRoleFromScopeFlag;
exports.applyGenericMoraCapitalization = applyGenericMoraCapitalization;
exports.classifyDisplayForToken = classifyDisplayForToken;
/**
 ...
 */
function decideEffectiveRoleFromScopeFlag(scopeFlag) {
 ...
 */
function applyGenericMoraCapitalization(core) {
    return core.moras
        .map((mora) => {
        const raw = mora.raw;
 ...
 * @returns 表示分類後のトークン
 */
function classifyDisplayForToken(token, repIndex) {
    // 対象外トークンはそのまま返す
    ...
    if (representative?.current) {
        const effectiveRole = decideEffectiveRoleFromScopeFlag(representative.scopeFlag);
        return {
            ...token,
            core: {
                ...token.core,
