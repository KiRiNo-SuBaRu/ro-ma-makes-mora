 ...
 * @returns MoraToken 列
 */
export function tokenizeToMoraTokens(jpRomaji: string): MoraToken[] {
 ...
 * @returns MoraUnit 列
 */
export function buildMoraUnits(tokens: MoraToken[]): MoraUnit[] {
  const result: MoraUnit[] = [];
  let i = 0;
...
// 各カテゴリの有無を集計する
export function summarizeMoras(units: MoraUnit[]): MoraSummary {
  // 拗音 CyV の判定
  const hasQ = units.some(
    (unit) => unit.kind === 'QCV' || unit.kind === 'QCyV'
  );

  // 通常の CV の判定
  const hasV = units.some(
    (unit) =>
      unit.kind === 'V' ||
      unit.kind === 'QCV' ||
      unit.kind === 'QCyV'
 ...
 */
export function buildMoraForToken(token: AnalyzedToken): AnalyzedToken {
...
if (token.kind !== 'JP_ROMAJI' || token.scope !== 'APPLICABLE') {
  ...
  const category: string =
    moraSummary.categories.length === 1
