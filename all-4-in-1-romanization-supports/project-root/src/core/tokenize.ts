...
function looksLikeJpRomaji(raw: string): boolean {
 ...
 * @returns 生セグメント列
 */
export function splitIntoRawSegments(input: string): string[] {
  const segments =
    input.match(/\s+|[\p{P}\p{S}]+|[^\s\p{P}\p{S}]+/gu);
 ...
 */
export function detectTokenKind(raw: string): TokenKind {
  if (raw.length === 0) {
    return 'UNKNOWN';
  }
 ...
 */
export function decideScopeKind(
  kind: TokenKind,
  fieldType: FieldType
): ScopeKind {
 ...
 */
export function tokenizeForMoraAnalysis(
  input: string,
  fieldType: FieldType
): AnalyzedToken[] {
